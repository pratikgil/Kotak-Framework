...
public class CommonMethods extends SetUp 
{
	...
	public static void switchToWindowById(String windowid)
	{
		try {
			...
			log.info("Swiched to window by id with ID "+windowid);
			
		}catch(Exception e) {
			log.error("Not able to switch to Window by ID "+e.getMessage());
		}
	}
...
public static boolean isTestRunnable(String testName, String sheetName) throws Exception
{
	...
	public static String readPropertyFile(String propertyName)throws UnhandledException, IOException
	{
		...
		FileInputStream fis =new FileInputStream("C:\\Users\\Pratik Gil\\eclipse-workspace\\MasterFramework-master\\MasterFramework\\src\\test\\resources\\PropertyFiles\\Config.properties");
	...
	public static void openLinkInNewTab(String locator) throws Exception
    {
        highLight(locator);
        try {
                WebElement element = driver.findElement(By.xpath(CommonMethods.readPropertyFile(locator)));
            ...
            log.error("Unable to open link in new tab due to "+e.getMessage());    
        }
    }
